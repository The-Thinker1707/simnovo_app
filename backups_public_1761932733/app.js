/**
 EduConnect prototype server (preserves API)
*/
...
app.use((req,res,next)=>{
  ...
  if(c){
    try{ const email = decodeURIComponent(c); const u = findUser(email); if(u) req.user = u; }catch(e){}
  }
  next();
});
...
app.post('/api/profile', upload.single('photo'), (req,res)=>{
  if(!req.user) return res.status(401).json({ error:'unauthenticated' });
  const d = load(); const u = (d.users||[]).find(x=>x.email===req.user.email);
  if(!u) return res.status(401).json({ error:'unauthenticated' });
  if(req.body.name) u.name = req.body.name; if(req.body.phone) u.phone = req.body.phone;
  if(req.file){ u.photo = req.file.filename; }
  save(d); res.json({ ok:true, user:u });
});
...
app.post('/api/lessons', (req,res)=>{
  ...
  const d = load(); d.lessons = d.lessons || []; const id = 'L-' + Date.now(); d.lessons.push({ id, title: req.body.title || 'Untitled', resource: req.body.resource || '' }); save(d); res.json({ ok:true });
});
...
app.get('/api/parent/dashboard', (req,res)=>{
  ...
  const d = load();
  const childId = req.user.childId || 's-1';
  const results = d.results && d.results[childId] ? d.results[childId] : d.results && d.results['s-1'] ? d.results['s-1'] : [];
...
app.post('/api/parent/complaint', (req,res)=>{
  ...
  const d = load(); d.complaints = d.complaints || []; const item = { id: 'c-'+Date.now(), parent: req.user.email, text, createdAt: new Date().toISOString() }; d.complaints.push(item); save(d); res.json({ ok:true, complaint:item });
});
...
app.post('/api/student/reflection', (req,res)=>{
  if(!req.user || req.user.role !== 'student') return res.status(403).json({ error:'forbidden' });
  const d = load(); d.reflections = d.reflections || {}; const sid = req.user.studentId || req.user.id || 's-1'; d.reflections[sid] = d.reflections[sid] || []; d.reflections[sid].push({ id: 'r-'+Date.now(), text: req.body.text || '', createdAt: new Date().toISOString() }); save(d); res.json({ ok:true });
});

// complaints
app.get('/api/complaints', (req,res)=>{ const d = load(); res.json(d.complaints || []); });

// admin
app.get('/api/admin/users', (req,res)=>{ const d=load(); res.json(d.users || []); });
...
// analytics: totals + grade summary
app.get('/api/analytics', (req,res)=>{
  ...
  (d.results_list||[]).forEach(r=>{
    ...
    else { const n = parseFloat(r.grade); if(!isNaN(n)) score = Math.max(0,Math.min(100,n)); }
    subjects[s] = subjects[s] || { total:0, count:0 };
    subjects[s].total += score;
    if(score>0) subjects[s].count += 1;
  });
...
app.listen(PORT, ()=>{ console.log('EduConnect (vivid interactive) running at http://127.0.0.1:' + PORT); });
